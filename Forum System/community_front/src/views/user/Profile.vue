      ...
      <div class="column is-one-quarter">
        <el-card shadow="never">
          <div slot="header" class="has-text-centered">
            <el-avatar :size="64" :src="require('@/assets/image/default.jpeg')" />
            <p class="mt-3">{{ topicUser.alias || topicUser.username }}</p>
          </div>
          <div>
            <p class="content">积分：<code>{{ topicUser.score }}</code></p>
            <p class="content">发表文章数：<code>{{ topicUser.topiccount }}</code></p>
            <p class="content">粉丝数：<code>{{ topicUser.followercount }}</code></p>
            <p class="content">入驻：{{ dayjs(topicUser.createTime).format("YYYY/MM/DD HH:MM:ss") }}</p>
            <p class="content">简介：{{ topicUser.job }}</p>
          </div>
        </el-card>
      </div>
